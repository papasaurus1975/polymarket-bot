...
def compute_metrics(trades: list[PaperTrade]) -> dict:
    """Return performance summary dict from a list of closed trades."""
    ...
    # Calibration: only trades with known outcomes
    calibrated = [t for t in closed if t.calibration_error is not None]
...
def _brier_score(trades: list[PaperTrade]) -> float:
    """Mean squared error between model probability and actual outcome (0 or 1)."""
    total = 0.0
    for t in trades:
...
def _empty_metrics() -> dict:
    return {
        "total_trades": 0, "open_trades": 0, "total_pnl": 0.0,
